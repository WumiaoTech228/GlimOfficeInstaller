...
namespace Office2024Installer
{
    ...
    public static class EmbeddedFont
    {
        ...
        // GetFont: 获取指定大小的字体对象
        // size: 字体大小（单位：像素或点，取决于GraphicsUnit）
        // FontStyle: 字体样式（Bold=粗体, Italic=斜体, Regular=常规等）
        public static Font GetFont(float size, FontStyle style = FontStyle.Bold)
        {
            FontFamily family = GetFontFamily();
            // Font: 字体对象，包含字体家族、大小、样式
    ...
    public class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            // DPI感知设置，确保在不同分辨率下显示效果一致
            // AutoScaleMode.Dpi: 根据DPI自动缩放
            this.AutoScaleMode = AutoScaleMode.Dpi;
            // AutoScaleDimensions: 基准DPI（96是100%缩放）
            ...
            try
            {
                ...
                using (Stream stream = assembly.GetManifestResourceStream("OfficeInstaller.icon.ico"))
                {
                    ...
                    {
                        // SystemIcons.Application: 系统默认应用图标
            ...
            this.Shown += (s, e) => {
                Logger.Info("主窗口显示完成。");
                GlimMessageBox.Show("程序已在同级目录下创建了 [GOI] 文件夹。\n\n所有下载文件、安装组件及日志均存储在此文件夹内，请勿在安装完成前随意删除该文件夹，否则会导致安装失败！", "重要提示");
            };
        }
        
        // MainForm_FormClosing: 窗口关闭事件处理
        // sender: 事件发送者
        // e: 事件参数，包含关闭原因等信息
        private void MainForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            // CloseReason.UserClosing: 用户点击关闭按钮
            if (e.CloseReason == CloseReason.UserClosing)
            {
                string message;
                string title;
                
                // 根据是否正在部署Office显示不同的提示
                if (isDeploying)
                {
                    // 正在部署时的警告提示
                    title = "正在部署Office";
                    message = "正在进行Office部署，您可能因为退出本程序未能正常安装并激活Office。\n\n确定要退出吗？";
                }
                else
                {
                    // 无任务时的友好提示
                    title = "真的要走了吗";
                    message = "真的要走了吗，来试试Glim Office Installer的功能吧！\n\n确定要退出吗？";
                }
                
                // GlimMessageBox.Show: 显示自定义消息框
                // MessageBoxButtons.YesNo: 显示是和否按钮
                // MessageBoxIcon.Question: 问号图标
                DialogResult result = GlimMessageBox.Show(message, title, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void InitializeComponents()
        {
            ...
            // 更新当前Y坐标，为下一个控件定位
            currentY = bannerHeight + 15;  // 根据banner高度计算
            ...
            versionLabels[1, 0] = createLabel(versionTexts[1, 0], 14, 28, Color.FromArgb(0, 90, 158));
            versionLabels[1, 1] = createLabel(versionTexts[1, 1], 11, 58, Color.FromArgb(60, 60, 60));
            versionLabels[1, 2] = createLabel(versionTexts[1, 2], 11, 86, Color.FromArgb(100, 100, 100));
